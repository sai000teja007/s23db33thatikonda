...
exports.cities_create_Page = function(req, res) {
console.log("create view")
try{
res.render('citiescreate', { title: 'cities Create'});
}
catch(err){
res.status(500)
res.send(`{'error': '${err}'}`);
...
exports.cities_update_Page = async function(req, res) {
    console.log("update view for item "+req.query.id)
    try{
    let result = await cities.findById(req.query.id)
    res.render('citiesupdate', { title: 'Cities Update', toShow: result });
    }
    catch(err){
    res.status(500)
    res.send(`{'error': '${err}'}`);
...
exports.cities_delete_Page = async function(req, res) {
    console.log("Delete view for id " + req.query.id)
    try{
    result = await cities.findById(req.query.id)
    res.render('citiesdelete', { title: 'cities Delete', toShow:
    result });
    }
    catch(err){
    res.status(500)
